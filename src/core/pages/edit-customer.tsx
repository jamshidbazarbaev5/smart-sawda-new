...
export default function EditCustomer() {
  ...
  useEffect(() => {
    if (customer) {
      ...
      setCustomerType(customer.customer_type ? String(customer.customer_type) : "");
      setCompanyName(customer.company_name || "");
      setEmail(customer.email || "");
      setCreditLimit(customer.credit_limit || "");
      setDiscountPercent(customer.discount_percent || "");
      setTaxId(customer.tax_id || "");
      setAddress(customer.address || "");
      setNotes(customer.notes || "");
      setSource(customer.source || "pos");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !phoneNumber.trim() || !id) {
  ...
  if (loadingCustomer) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:bg-card flex items-center justify-center">
