...
import { useUpdateVehicleIncome, fetchVehicleIncomeDetails } from '../api/vehicle-income';
...
export default function EditVehicleIncome() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const updateVehicleIncome = useUpdateVehicleIncome();
  ...
  // Load vehicle income details
  useEffect(() => {
    const loadIncome = async () => {
      if (id) {
        try {
          const income = await fetchVehicleIncomeDetails(parseInt(id));
          setVehicleId(income.vehicle.toString());
          setDriverId(income.driver.toString());
          setStoreId(income.store.toString());
          setDate(income.date);
          setComment(income.comment || '');
          setPayments(income.payments || [{ payment_method: 'Наличные', amount: '' }]);
          // Store worker name from current user
          setWorkerName(currentUser?.name || '');
        } catch (error) {
          toast.error(t('messages.error.not_found', { item: t('navigation.vehicle_incomes') }));
 ...
 setVehicleId((newVehicle?.id ?? '').toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateVehicleIncome.mutateAsync(incomeData);
      toast.success(t('messages.success.updated', { item: t('navigation.vehicle_incomes') }));
      navigate('/vehicle-incomes');
    } catch (error) {
      toast.error(t('messages.error.update', { item: t('navigation.vehicle_incomes') }));
    }
  };
  ...
  return (
    <div className="container py-8 px-4">
      <h1 className="text-2xl font-bold mb-6">{t('common.edit')} {t('navigation.vehicle_incomes')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <div className="flex gap-4 pt-4">
          <Button
            type="submit"
            disabled={updateVehicleIncome.isPending}
            className="flex-1"
          >
            {updateVehicleIncome.isPending ? t('common.updating') : t('common.update')}
          </Button>
          <Button
            type="button"
            variant="outline"
